...
<a id='intake'></a>
## Снятие заявки
Старт работы по позиции начинается с получения заявки на вакансию (может быть в форме письма и/или заполненного документа). После этого, необходимо провести встречу с нанимающей командой, для уточнения всех деталей связанных с проектом/командой и поиском, для формирования профиля кандидата.  Обычно для этого используют готовый чек-лист с вопросами, например такая [форма для снятия заявки](https://drive.google.com/file/d/1YQDQNdWLylDZR3KxPbknymA7sdDq3iXY/view?usp=sharing).
...
Предположим, что вы параллельно занимаетесь запуском реферальной программы, обработкой внутренних кандидатов (ротация сотрудников, сотрудники, не задействованные на проектах, кандидаты из внутернних программ подготовки),
...
закрытые позиции, которые были отмечены как перспективные и которым теперь можно вернуться. Этих
...
Rabota.by - это самый крупный job board и база «горячих» кандидатов в РБ (это тот же [hh.ru](https://hh.ru), только региона Беларуси). 
...
Linkedin – наверное, самая известная профессиональная социальная сеть в мире. На март 2021 года в ней зарегистрировано около 500 000
...
состоит из многочисленных итераций. Прорабатывая последовательно запрос за запросом, мы двигаемся
итерациями, чтобы охватить максимально широкую выборку и найти ~~единорога~~ идеального кандидата.
### Итеративный поиск
...
1. Кандидаты, которые подходят по признаку «A»; Например: 
    - SQL AND (Python OR R) AND (ml OR mining OR <i>keyword3</i> OR <i>keyword4</i>)
2. Кандидаты, которые подходят по признаку «Б»; Например: 
    - SQL AND (Python OR R) AND (tableau OR "Power BI" OR *keyword3*) AND NOT (ml OR mining) – можем глянуть профайлы тех, кто не указывал часть скиллсета, но теоретически может быть нам интересен
3. Кандидаты, которые подходят по опыту и тайтлу
4. Кандидаты, которые частично соответствуют требованиям по позиции

### [Гайд по булевому поиску в Linkedin](https://drive.google.com/file/d/1PCmeWfeRZ4Sc1fZt2YHLlAeVTw00HJo4/view?usp=sharing)
...
- `site:stackoverflow.com/cv Belarus python (django OR flask OR tornado OR redis OR rabbitmq)`
...
<a id='reporting'></a>
## Отчетность
Отчеты показывает не только результаты активности за предыдущий период, но и отражают проблемные зоны, что позволяет оперативно перераспределить ресурсы.
...
<img src="https://github.com/carrollstreet/Recruiting-Guide/blob/main/charts/recruiting_funnel.png" />
 ...
 > - На втором проблема между этапами `test task` и `interview`. 80% кандидатов уходят на этапе тестового задания, например, по причине высокой сложности тестового задания или недостаточной мотивации на его выполнение. Решением может быть пересмотр требований либо предложение тестового после личного интервью
...
Company Mapping/Talent Mapping - виды исследовательской аналитики. Отличается от анализа емкости тем, что направлена не на приблизительную оценку размеров рынка, а на поиск конкретной информации о компаниях (числе целевых сотрудников в них), формирование пулов кандидатов и т. д.
...
Пример визуализации распределения числа релевантных кандидатов в целевых компаниях на случайно сгенерированных данных:
